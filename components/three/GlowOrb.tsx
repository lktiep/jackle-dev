...
// Animated glowing orb with noise-driven surface displacement
export default function GlowOrb() {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uColor2: { value: new THREE.Color('#00ddff') },
      uColor3: { value: new THREE.Color('#8855ff') },
    }),
    []
  );
  ...
  const vertexShader = `
    ...
    void main() {
      vUv = uv;
      float noise = snoise(position * 1.8 + uTime * 0.4);
      float noise2 = snoise(position * 3.0 + uTime * 0.6) * 0.5;
      vDisplacement = noise + noise2;
      vec3 newPosition = position + normal * (noise + noise2) * 0.15;
